...
contract StableCoinFactory is Ownable {
        ...
        returns (address)
    {
        ...
        address newToken =
            address(new StableCoin(name, symbol, initialSupply, defaultAdmin, pauser, minter, burner, decimal));
        emit NewStablecoin(reserveAddress, newToken, false);

        deployedStablecoins.push(newToken);
        arrayOfStableCoins.push(
            Stablecoin(
                name,
                symbol,
                initialSupply,
        ...
        stableCoinDetails[newToken] = Stablecoin(
            name, symbol, initialSupply, maxSupply, defaultAdmin, pauser, minter, burner, decimal, feedAddress, newToken
        );
        return (newToken);
    }
